...
#	<type 'str'>
	...
	if intraday == False:
		...
		for sym in symbol:
			...
			urlToRead = "http://chart.finance.yahoo.com/table.csv?s="+sym+"&a="+a+"&b="+b+"&c="+c+"&d="+d+"&e="+e+"&f="+f+"&g=d&ignore=.csv"
						 #~ http://chart.finance.yahoo.com/table.csv?s=YHOO&a=11&b=15&c=2016&d=0&e=15&f=2017&g=d&ignore=.csv
...
def data_slicer(symbol, date_day, interval, onefile = 'off',
				inputFileName = None, 
...
#~  pyzhance.data_slicer(symbol = ['DIS'], date_day = [('01-26','07-24')], interval = [('2010-01-22','2016-07-01')])
	...
	for inputs in symbol:
		...
		('close','f3'),('volume','i8'),('adjclose','f3')],converters = {0:date_converter})
			...
			for day in date_day:
				if isinstance(day, tuple) == True:	#	this line checks to see if the day object is a list.
					...
					if len(startdate3) ==  10:
						...
						index = []
						if enddate5 <= dic1['date_{}'.format(inputs)][0]:
							while len(index) < 2:
							...
							if onefile == 'on':
								...
								ftw = open(inputs+'_'+str(startdate4.date())+'_'+str(enddate4.date()), 'w')
								for count2 in range(index[0][0], index[1][0] - 1, -1):
									ftw.write('{},{},{:011.6f},{:011.6f},{:011.6f},{:011.6f},{:011.6f},{}\n'.format(
									datetime.date.fromordinal(dic1['date_{}'.format(inputs)][count2]), 
									dic1['date_{}'.format(inputs)][count2], dic1['open_{}'.format(inputs)][count2], 
						...
						elif enddate5 > dic1['date_{}'.format(inputs)][0]:
							break
